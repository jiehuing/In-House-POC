...
public partial class GeneratePdfPrestige : System.Web.UI.Page
{
        string regionName;
    ...
    protected void Page_Load(object sender, EventArgs e)
        {
                    lblSearchData.Text = "Prestige Fund Fact Sheet";

        //DateTime date = DateTime.Now;
        region = "LU172002";
        SqlConnection con = new SqlConnection(connectionString);
        string selectGLD = "select top 1 * from main where ISIN= '" + region + "' order by date DESC";
        SqlCommand gLD = new SqlCommand(selectGLD, con);
        con.Open();
        using (SqlDataReader read = gLD.ExecuteReader())
        {
            while (read.Read())
            {
                date = (DateTime)read["date"];
                textGetLatestDate.Text = date.ToString();
                Debug.WriteLine("in while loop");
            }
        }
        con.Close();
        dtDate = Convert.ToDateTime(textGetLatestDate.Text);
        dtp = dtDate.ToString("yyyy-MM-dd");
        dtp = textGetLatestDate.Text;
        

        //dtp = date.ToString("yyyy-MM-dd");

        Debug.WriteLine("dtp is " + dtp);
                    
        lblSearchDate.Text = "Date: " + date.ToString("dd/MM/yyyy");
        Debug.WriteLine(lblSearchDate.Text);
        ...
        */
        ...
        string oya = date.AddYears(-1).ToString("yyyy/MM/dd");
                    ...
                    string selectSearchDate = "select * from main where Date ='" + dtp + "' AND ISIN = '" + region + "'";
                    Debug.WriteLine(selectSearchDate);
                    string selectOneYearAgo = "select top 1 * from main where MONTH(Date) = '" + oyaList[1] + "' AND YEAR(date) = '" + oyaList[0] + "' AND ISIN='" + region + "' order by Date DESC"; //AND ISIN='" + region + "'  order by Date DESC";
                    string selectSixMonthsAgo = "select top 1 * from main where MONTH(Date) = '" + smaList[1] + "' AND YEAR(date) = '" + smaList[0] + "' AND ISIN='" + region + "' order by Date DESC";
                    string selectThreeMonthsAgo = "select top 1 * from main where MONTH(Date) = '" + tmaList[1] + "' AND YEAR(date) = '" + tmaList[0] + "' AND ISIN='" + region + "' order by Date DESC";
                    string selectOneMonthsAgo = "select top 1 * from main where MONTH(Date) = '" + omaList[1] + "' AND YEAR(date) = '" + omaList[0] + "' AND ISIN='" + region + "' order by Date DESC";
                    string selectSinceInception = "select top 1 * from main where ISIN= '" + region + "' order by date";
                    string selectInceptionYear = "select top 1 YEAR(date) AS Date from main where ISIN= '" + region + "' order by date";
                    string selectInceptionMonth = "select top 1 MONTH(date) AS Date from main where ISIN= '" + region + "' order by date";
                    string selectYearToDate = "select top 1 * from main where YEAR(date) = '" + ytdList[0] + "' AND ISIN= '" + region + "' order by Date"; //Remove DESC if need to be first day of the year
                    string selectTop5Holding = "select TOP 5 companyname, sector, fixedHoldingRatio from companyinfo where status = 'alive' AND ISIN= '" + region + "' order by fixedHoldingRatio DESC, companyName";
                    string selectSector = "SELECT sector, SUM(fixedHoldingRatio) Percentage FROM companyInfo where status = 'Alive' and ISIN = '"+ region +"' GROUP BY sector ORDER BY Percentage";
                    string selectPastFundDetail = "select * from pastfundDetail where date <= '" + dtp + "' and ISIN= '" + region + "'  order by date DESC";
                    ...
                    SqlCommand pastFundDetail = new SqlCommand(selectPastFundDetail, con);
                    ...
                    string fundSizeValue;
                    string baseCurrencyValue;
                    int numberOfHoldingsValue;
                    string initialFeeValue;
                    string EISValue;
                    string allInFeeValue;
                    string tERValue;
                    string unitNAVValue;
                    ...
                    using (SqlDataReader read = pastFundDetail.ExecuteReader())
                    {
                        while (read.Read())
                        {
                            benchmarkValue = (string)read["benchmarkName"];
                            fundManagerValue = (string)read["fundManagerName"];
                            fundSizeValue = (string)read["fundSize"];
                            baseCurrencyValue = (string)read["baseCurrency"];
                            numberOfHoldingsValue = (int)read["noOfHolding"];
                            initialFeeValue = (string)read["initalFee"];
                            EISValue = (string)read["EIS"];
                            allInFeeValue = (string)read["AIS"];
                            tERValue = (string)read["TER"];
                            unitNAVValue = (string)read["uint"];
                    ...
                    Debug.WriteLine("searchYear " + searchYear);
        Debug.WriteLine("textInceptYear " + textInceptYear.Text);
        Debug.WriteLine("textInceptYear " + Int32.Parse(textInceptYear.Text).ToString());
                    ...
                    for (int i = 0; i <= totalMonths; i++)
                    {
                        ...
                        string selectEveryMonth = "select top 1 * from main where YEAR(date) = '" + siIntYear + "' AND MONTH(date) ='" + siIntMonth + "' AND ISIN= '" + region + "' order by Date DESC";
        ...
        for (int i = 0; i < sectorList.Count; i++)
                    {
                            Debug.WriteLine(Convert.ToDecimal(percentList[i]));
                            u.Points.AddXY(sectorList[i].ToString(), Convert.ToDecimal(percentList[i])); //
                    }
                    ...
                    totalPercent = Convert.ToDecimal(top5List[2]) + Convert.ToDecimal(top5List[5]) + Convert.ToDecimal(top5List[8]) + Convert.ToDecimal(top5List[11]) + Convert.ToDecimal(top5List[14]);
